...
<header>
    ...
    <script>
		document.querySelectorAll('.select_btn').forEach(element => element.onclick = function(event) {
			...
			if(name != '' && price != '') {
				opener.document.getElementById('review_write_img').style.backgroundImage = 'url(' + img.value + ')'
				
				opener.document.getElementById('product_info_name').innerText = name.value
				opener.document.querySelector('.product_info_name').value = name.value
				
				opener.document.getElementById('product_info_price').innerText = price.value
				opener.document.querySelector('.product_info_price').value = price.value				
